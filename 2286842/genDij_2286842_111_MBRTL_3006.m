...
cs_coll = 7;  % center-to-center distance (mm) 3-7
angle_coll = 90;
if angle_coll == 0
...
cy = y_min : cs_coll : y_max;
ay = cy - w_coll(1);
by = cy + w_coll(1);
ax = x_min * ones(size(ay));
bx = x_max * ones(size(by));
end
...
save(['dij_' ptid '_111_collimator' num2str(angle_coll) '_ctc' num2str(cs_coll) 'mm_' num2str(pln.propStf.gantryAngles) '1.mat'],'dij','stf','-v7.3')
end