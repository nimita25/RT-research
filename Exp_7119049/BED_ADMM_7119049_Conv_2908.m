...
N_obj = 7;
c_obj = [1; 1; 1; 3; 3;  4; 5; ];
type_obj = [0;5;8;2;3; 3; 2; ];
s_obj = [px;1.1*px;px;18;12;27;20;];
n_obj = [nan;nan;0.95;nan;0.3;0.6;nan;];
w_obj = [1;1;1;1;1;1;1;];

% N_obj = 9;
% c_obj = [1; 1; 1; 3; 3;  4; 5; 4; 5;];
% type_obj = [0;5;8;2;3; 3; 2; 0; 0;];
% s_obj = [px;1.1*px;px;18;12;27;20;0;0;];
% n_obj = [nan;nan;0.95;nan;0.3;0.6;nan;nan;nan;];
% w_obj = [1;1;1;1;1;1;1;0.01;0.01;];
...
if exist(fname,"file")
    ...
    % end
    for i = 1:min(nfrac,unique_fields)
        Cost_matrix{i} = sparse(size(dij.physicalDose{1}, 1),48646);
        tmp_n = 0;
        for j = 1:numel(arc_id{i})
            load([folder ptid '_' num2str(arc_id{i}(j)*15) '.mat'], 'dij');
            disp('load');
            disp(arc_id{i}(j));
            nX = size(dij.physicalDose{1},2);
            for jj = 1:numel(c)
                if jj ~= 2
                    Cost_matrix{i}(c{jj},tmp_n + (1:nX)) = dij.physicalDose{1}(c{jj},:); %Get rows corresponding to tumor voxels, and columns corresponding to arcs
                end
            end
            tmp_n = tmp_n + nX;
        end
    ...
    save(fname,"Cost_matrix","num_beams",'-v7.3');
end
