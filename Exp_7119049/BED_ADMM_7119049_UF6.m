...
%gamma = zeros(size(y));
gamma = y;
z_n0 = zeros(numel(c{1}),N_obj);
lambd_n0 = zeros(numel(c{1}),N_obj);
z_min = zeros(numel(c{1}),N_obj);
lambd_min = zeros(numel(c{1}),N_obj);
z_max = cell(n_rho,1);
...
id_min = cell(N_obj,1); % set of active indices of DVH min in each fraction
...
for i = 1:n_rho
    for ll = 1:length(type_constr{i})
        if type_constr{i}(ll) == 1
            z_max{i} = zeros(n_c(i),N_obj);
            lambd_max{i} = zeros(n_c(i),N_obj);
            n = 0;
            for j = 1:N_obj
        ...
        elseif type_constr{i}(ll) == 2
            z_mean{i} = zeros(n_c(i),N_obj);
            lambd_mean{i} = zeros(n_c(i),N_obj);
            n = 0;
            for j = 1:N_obj
                tmp_index = size(tmp_Dij{mod(j-1,unique_fields)+1,i},2);
        ...
        elseif type_constr{i}(ll) == 3
            z_dvh{i} = zeros(n_c(i),N_obj);
            lambd_dvh{i} = zeros(n_c(i),N_obj);
            n = 0;
            for j = 1:N_obj
                tmp_index = size(tmp_Dij{mod(j-1,unique_fields)+1,i},2);
    ...
    for nm = 1:n_rho
        for ll = 1:length(type_constr{nm})
            if type_constr{nm}(ll) == 1
                ...
                p = 1 - sqrt(((BED_max(nm)/rho(nm))+(N_obj*RBE^2/(2*rho(nm))^2))./sum((tmp_prod+(RBE/(2*rho(nm)))).^2,2));
                % disp(min(p));
                % disp(max(p));
                p = max(0,p);
                %disp(size(p));
            ...
            elseif type_constr{nm}(ll) == 2
                ...
                p = 1 - sqrt(((BED_mean(nm)*n_c(nm)/rho(nm))+(n_c(nm)*N_obj*RBE^2/(2*rho(nm))^2))/sum((tmp_prod+(RBE/(2*rho(nm)))).^2,"all"));
                % disp(min(p));
                % disp(max(p));
                p = max(0,p);
        ...
        if any(type_constr{nm} == 3)
            for nd = 1:num_DVH(nm)
                if ~isempty(id{nm}{nd})
                    tmp_prod = zeros(numel(id{nm}{nd}),N_obj);
                    %tt = zeros(n_c(nm,1));
                    n = 0;
                    for nf = 1:N_obj
                        tmp_index = size(tmp_Dij{mod(nf-1,unique_fields)+1,nm},2);
                    ...
                    p = 1 - sqrt(((BED_DVH_max{nm}(nd)/rho(nm))+(N_obj*RBE^2/(2*rho(nm))^2))./sum((tmp_prod+(RBE/(2*rho(nm)))).^2,2));
                    % disp(min(p));
                    % disp(max(p));
                    p = max(0,p);
                    %disp(size(p));
