...
n_rho = numel(oar)+2;
rho = zeros([n_rho,1]);
rho(1) = cst{38,5}.betaX/cst{38,5}.alphaX;
rho(2) = cst{1,5}.betaX/cst{1,5}.alphaX;
rho(3) = cst{37,5}.betaX/cst{37,5}.alphaX;
rho(4) = cst{11,5}.betaX/cst{11,5}.alphaX;
rho(5) = cst{12,5}.betaX/cst{12,5}.alphaX;
rho(6) = cst{13,5}.betaX/cst{13,5}.alphaX;
rho(7) = cst{44,5}.betaX/cst{44,5}.alphaX;
...
N_obj = 8;
c_obj = [1; 1; 1; 3; 4; 5; 6; 7;];
type_obj = [0;5;8;1;1;1;1;1;];
s_obj = [px;1.1*px;px;[54;36;54;54;60]];
n_obj = [nan;nan;0.95;nan;nan;nan;nan;nan;];
w_obj = [1;1;100;1;1;1;1;1];

% w_obj = [1; 100; 1; 0.1; 1; 1; 1; 1; 1; 1;];
% s_obj = [px; px; px * 1.1; 0; px; [15; 10; 10; 10; 12]/nfrac];
% n_obj = round([nan; n_c(1) * 0.95; nan; nan; nan; nan; nan; nan; nan; 5 / 0.3^3;]);
% c_obj = [1; 1; 1; 2; 2; 3; 4; 5; 6; 7;];
% id_obj = cell(N_obj, 1);
% type_obj = [0; 2; 3; 0; 3; 3; 3; 3; 3; 1;];
...
out_folder = "output/";
resd = cell(3,1);
% fnames = {'./output/BED-ADMM-2024-08-31-18-50.mat' ...
%     './output/BED-ADMM-2024-08-31-19-23.mat',...
%     './output/BED-ADMM-2024-08-31-18-15.mat'};
fnames = {'./output/BED-ADMM-2024-08-31-18-15.mat' ...
    './output/BED-ADMM-2024-08-31-18-50.mat'};
...
for ff = 1:numel(fnames)
    ...
    % Get output metrics
    [D95, Dmax, CI, BEDmax_BS, BEDmean_BS, BEDmax_OC, BEDmean_OC,...
    BEDmax_ON, BEDmean_ON, BEDmax_B, BEDmean_B] = calcparaBED_9306087(MD, BED2, px, c);
    resd{ff} = [D95, Dmax, CI, BEDmax_BS, BEDmean_BS, BEDmax_OC, BEDmean_OC,...
    BEDmax_ON, BEDmean_ON, BEDmax_B, BEDmean_B];
...
% legend show
legend(L([2,6]), 'Brainstem','Brain','Location','northeast','FontSize',14)
grid on
...
% grid on
% %title(strcat('BED DVH in brainstem for case ID: ',ptid));
% saveas(gcf,strcat(out_folder,'BEDBrainstem_2908_',ptid,'.fig'));
% hold off
