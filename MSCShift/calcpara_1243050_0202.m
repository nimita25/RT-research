function [D95, Dmax, CI, Dmax_oar1, V25_oar1, Dmax_oar2, V20_oar2, Dmax_oar3, V25_oar3, Dmax_oar4, V12_oar4, Dmean_body] = calcpara_1243050_0202(nfrac, d, px, c)
...
V25_oar1 = sum(d(oar1) >= 25 / nfrac) / numel(oar1) * 100; % Percent volume receiving at least 25 Gy

Dmax_oar2 = max(d(oar2));
V20_oar2 = sum(d(oar2) >= 20 / nfrac) / numel(oar2) * 100; % Percent volume receiving at least 20 Gy

Dmax_oar3 = max(d(oar3));
V25_oar3 = sum(d(oar3) >= 25 / nfrac) / numel(oar3) * 100; % Percent volume receiving at least 25 Gy

Dmax_oar4 = max(d(oar4));
V12_oar4 = sum(d(oar4) >= 12 / nfrac) / numel(oar4) * 100; % Percent volume receiving at least 12 Gy
