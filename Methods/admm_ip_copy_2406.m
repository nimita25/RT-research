...
for n_iter = 1:N_iter
    ...
    %     end
    % end
    A_reduced = zeros(size(var_CG.var_AtA.Dij{1},1),ip.ns);
    for k = 1:size(var_CG.var_AtA.Dij{1},1)
        A_reduced(k,:) = Wx(full(var_CG.var_AtA.Dij{1}(k,:))',wp)';
    end
    A = A_reduced'*A_reduced+double(mu)*speye(size(A_reduced,2));
    var_CG.var_AtA.Dij{1} = full(A_reduced);
    b = AtmX(rhs,var_CG.var_AtA); %Get ((D times x)^T.W)b
    b_lin = b+mu*(z-u);
    b_lin = sparse(double(b_lin));
    A = full(A);
