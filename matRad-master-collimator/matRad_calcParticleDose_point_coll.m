function dij = matRad_calcParticleDose_point_coll(ct,stf,pln,cst,calcDoseDirect)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
dij.point.DosePoint = pln.DosePoint;
dij.point.numOfVoxels = size(pln.DosePoint,1);
...
for i = 1:dij.numOfScenarios
    dij.physicalDose{i} = spalloc(dij.point.numOfVoxels,numOfColumnsDij,1);
end
...
for i = 1:length(stf) % loop over all beams
    ...
    rot_DosePoint = dij.point.DosePoint*rotMat_system_T;

    rot_coordsV(:,1) = rot_coordsV(:,1)-stf(i).sourcePoint_bev(1);
    rot_coordsV(:,2) = rot_coordsV(:,2)-stf(i).sourcePoint_bev(2);
    rot_coordsV(:,3) = rot_coordsV(:,3)-stf(i).sourcePoint_bev(3);

    rot_DosePoint(:,1) = rot_DosePoint(:,1)-stf(i).sourcePoint_bev(1);
    rot_DosePoint(:,2) = rot_DosePoint(:,2)-stf(i).sourcePoint_bev(2);
    rot_DosePoint(:,3) = rot_DosePoint(:,3)-stf(i).sourcePoint_bev(3);

    mgx=(1:ct.cubeDim(1))*ct.resolution.x-stf(i).isoCenter(1);
    mgy=(1:ct.cubeDim(2))*ct.resolution.y-stf(i).isoCenter(2);
    mgz=(1:ct.cubeDim(3))*ct.resolution.z-stf(i).isoCenter(3);
    ...
    radDepthVpoint = matRad_interpRadDepth(ct,mgx,mgy,mgz,1,V,dij.point.DosePoint,radDepthV);
    fprintf('done.\n');
    
    % get indices of voxels where ray tracing results are available
    radDepthIx = find(~isnan(radDepthV{1}));
    radDepthpointIx = find(~isnan(radDepthVpoint{1}));
    
    % limit rotated coordinates to positions where ray tracing is availabe
    rot_coordsV = rot_coordsV(radDepthIx,:);
    rot_DosePoint = rot_DosePoint(radDepthpointIx,:);
    ...
    for j = 1:stf(i).numOfRays % loop over all rays
        ...
        if ~isempty(stf(i).ray(j).energy)
            ...
            % [ix,radialDist_sq] = matRad_calcGeoDists(rot_DosePoint, ...
            %                                          stf(i).sourcePoint_bev, ...
            %                                          stf(i).ray(j).targetPoint_bev, ...
            %                                          machine.meta.SAD, ...
            %                                          radDepthpointIx, ...
            %                                          maxLateralCutoffDoseCalc);
            [ix,rot_coordsVpoint_coll, radialDist_sq] = matRad_calcGeoDists_coll(rot_DosePoint, ...
                                                     stf(i).sourcePoint_bev, ...
                                                     stf(i).ray(j).targetPoint_bev, ...
                                                     machine.meta.SAD, ...
                                                     radDepthpointIx, ...
                                                     maxLateralCutoffDoseCalc); % ix is in index in patient

            radDepths = radDepthVpoint{1}(ix);
            ...
            for k = 1:stf(i).numOfBixelsPerRay(j) % loop over all bixels per ray
                ...
                bixelDose = matRad_calcParticleDoseBixel_coll(...
                    stf(i).ray(j), stf(i).ray(j).ratio(k), currRadDepths, ...
                    rot_coordsVpoint_coll(currIx,:), ...
                    sigmaIni_sq, ...
                    machine.data(energyIx), pln);   
                ...
                doseTmpContainer{mod(counter-1,numOfBixelsContainer)+1,1} = sparse(ix(currIx),1,bixelDose,dij.point.numOfVoxels,1);
