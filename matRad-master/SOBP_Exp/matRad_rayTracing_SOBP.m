function [radDepthV,geoDistV] = matRad_rayTracing_SOBP(stf,ct,V,rot_coordsV,lateralCutoff)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:size(rayMx_world,1)
    ...
    if any(ixRememberFromCurrTracing) > 0
        ...
        for j = 1:ct.numOfCtScen
            ...
            % [~,l_RF,rho_RF,~,~] = matRad_siddonRayTracer(stf.isoCenter, ...
            %     ct.resolution, ...
            %     stf.sourcePoint, ...
            %     rayMx_world(i,:), ...
            %     ct_RF.cube);
            % if sum(l_RF.*rho_RF{1,1}) ~=0
            %     sum(l_RF.*rho_RF{1,1})
            % end

            % Calculate accumulated d sum.
            dCum = cumsum(d)-d/2;
            % dCum = cumsum(d)-d/2 + sum(l_RF.*rho_RF{1,1});
