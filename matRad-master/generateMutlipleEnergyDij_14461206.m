...
delta=5;dx=3;
% ptid='51426287'; % (!)
load(['C:\Users\hao\Desktop\FLASH\' ptid '\' ptid '.mat'],'cst','ct');
idctv=[9]; % (!)
...
energy = [machine.data.energy];
% Spec_energy = [machine.data(24).energy machine.data(108).energy];
...
%%
for ie=1:numel(energy)
[dij_cell,stf_cell] = generateMulti_dij(ptid,pln,ct,cst_ctv,cst_ptv,machine,energy(ie),bw,lss,gantryAngles);
end
...
%     [stf,~] = matRad_generateStf_maxenergy(ct,cst_ctv,pln,0);
    stf = matRad_generateStf_multienergy270620(ct,cst_ctv,pln,0);
    stf_cell{i} = stf;
    %%
    shift=[0 0 0];
    stf2=stf;
...
%         save([ptid_1 '\','51426287_dij_proton_',num2str(bw),num2str(lss),'_Spec_energy_',num2str(i),'.mat'],'dij','-v7.3')
        save(['C:\Users\hao\Desktop\FLASH\' ptid '\' ptid '_dij_',num2str(i),'_',num2str(round(Spec_energy)) '.mat'],'dij','-v7.3')
%         save(['C:\Users\hao\Desktop\FLASH_BL\' ptid '\' ptid '_dij_',num2str(i),'_v2.mat'],'dij','stf','-v7.3')
    end
end
