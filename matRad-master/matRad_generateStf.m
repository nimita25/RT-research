...
if strcmp(pln.radiationMode,'protons') || strcmp(pln.radiationMode,'carbon')
    ...
    availablePeakPos  = [machine.data.peakPos] + [machine.data.offset]; % what is offset
...
for i = 1:length(pln.propStf.gantryAngles)
    ...
    if pln.propStf.bixelWidth < maxCtResolution
     ...
     % remove spaces within rows of bixels for DAO (revise)
    ...
    % Save ray and target position in lps system. (revise)
    for j = 1:stf(i).numOfRays
    ...
    for j = stf(i).numOfRays:-1:1
       ...
       if strcmp(stf(i).radiationMode,'protons') || strcmp(stf(i).radiationMode,'carbon')
           ...
           if sum(rho{2}) > 0 
                ...
                for k = 1:numel(targetEntry) % why target Entry is more than one
                ...
                currentMinimumFWHM = matRad_interp1(machine.meta.LUT_bxWidthminFWHM(1,:)',...
                                             ...
                                             pln.propStf.bixelWidth);  % what is this
                focusIx  =  ones(stf(i).numOfBixelsPerRay(j),1); % what is focusIx
                ...
                for k = 1:stf(i).numOfBixelsPerRay(j)                    
                    focusIx(k) = find(machine.data(vEnergyIx(k)).initFocus.SisFWHMAtIso > currentMinimumFWHM,1,'first'); % what is this
                end
