function stf = matRad_generateStf_112018_v2(lss,ct,cst,pln,visMode)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if strcmp(pln.radiationMode, 'protons') || strcmp(pln.radiationMode, 'carbon')
        for j = 1:stf(i).numOfRays
            for k = 1:numel(stf(i).ray(j).energy)
                ...
                stf(i).ray(j).rangeShifter(k).eqThickness = 4;
                stf(i).ray(j).rangeShifter(k).sourceRashiDistance = 3;
            end
        end
    end
