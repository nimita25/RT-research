function stf = matRad_generateStf_multienergy270620(ct,cst,pln,visMode)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                     stf(i).ray(j).energy = [stf(i).ray(j).energy availableEnergies(availablePeakPos>=targetEntry(k)&availablePeakPos<=targetExit(k))];
                     stf(i).ray(j).energy = pln.propStfSpec_energy;
                end
    ...
    if strcmp(stf(i).radiationMode,'protons') || strcmp(stf(i).radiationMode,'carbon')
        ...
        if strcmp(machine.meta.machine,'Generic')
            longitudinalSpotSpacing = pln.propStf.longitudinalSpotSpacing; % enforce all entries to be used1.5
        else
            longitudinalSpotSpacing = 3;   % default value for all other treatment machines 3
        end
