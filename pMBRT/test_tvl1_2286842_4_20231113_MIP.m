...
for i_id=1:nN
    for i_ctc=1:numel(ctc)
        load([folder 'dij_' ptid '_collimator_beam4_45deg+90_ctc' num2str(ctc(i_ctc)) 'mm_b3_doseGrid113_' num2str(id(i_id)) '.mat']);
        all_Dij(i_id,i_ctc) = Dij;
        mean_Dij(i_id,i_ctc) = mean(Dij{1}(:));
    end
end
