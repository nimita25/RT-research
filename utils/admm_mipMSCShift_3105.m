function [xp,y] = admm_mipMSCShift_3105(ip,var_CG,x_init,qc)
%% Parameters set up
mu_min = ip.mu_min;
eps = mu_min/2;
...
y = ones(n_ba,1,'single')/numel(wp.shifts);%ones(n_ba,1,'single')*N/n_ba;
%y = zeros(n_ba,1);y(3)=1;y(7)=1;y(11)=1;y(15)=1;
u = zeros(numel(wp.id),1);
n = 0;
for i_id=1:numel(wp.id)
    u(i_id) = sum(y(n+(1:numel(wp.shifts))))-1;
    n = n+numel(wp.shifts);
end
disp(u');
%u = y;
...
for n_iter = 1:N_iter
    ...
    BLKD = ones(numel(wp.shifts),numel(wp.shifts));
    for i_id = 1:numel(wp.id)-1
        BLKD = blkdiag(BLKD,ones(numel(wp.shifts),numel(wp.shifts)));
    end
    ...
    A_lhs = DTD+mu*BLKD;
    b_rhs = b+mu*(ones(n_ba,1)-repelem(u,numel(wp.shifts)));
    disp(b');
    disp(b_rhs');
    % disp(DTD);
    % disp(mu);
    ...
    n = 0;
    for i_id=1:numel(wp.id)
        u(i_id) = u(i_id)+sum(y(n+(1:numel(wp.shifts))))-1;
        n = n+numel(wp.shifts);
    end
    disp(u');
